...
async function readCount() {
  try {
    ...
    const count =
      typeof parsed?.count === "number" ? parsed.count : seedCount;

    return { count, synced: true };
  } catch (error) {
    // Si el archivo no existe, intentamos crearlo con el valor semilla
    if (error.code === "ENOENT") {
      try {
        await fs.mkdir(path.dirname(filePath), { recursive: true });
        await fs.writeFile(
          filePath,
          JSON.stringify({ count: seedCount }, null, 2),
          "utf-8"
        );
    ...
    // FS de solo lectura u otros códigos restringidos
...
async function writeCount(count) {
  try {
    ...
    if (READ_ONLY_ERROR_CODES.has(error.code)) {
      console.warn("Filesystem is read-only, skipping write.", error);
      return;
    }
    console.error("Error writing downloads count:", error);
    throw error;
  }
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: "Unable to read downloads." },
      { status: 500 }
    );
  }
}
...
export async function POST() {
  ...
  try {
    ...
    // Si el FS es solo lectura, devolvemos el valor actualizado pero marcando synced=false
    ...
    return NextResponse.json(
      { error: "Unable to update downloads." },
      { status: 500 }
    );
  }
}
